...
const CartProduct = ({ id, name, image, category, qty, total, price }) => {
    ...
    const sendData = async()=>{
        ...
        fetch(`${process.env.REACT_APP_SERVER_DOMAIN}/cart`, {
            method: "POST",
            headers: {
        ...
        <div className='bg-slate-200 p-2 flex gap-4 rounded border  border-slate-300'>
            ...
            <div className='px-3 flex flex-col gap-2 w-full border '>
                <div className='flex justify-between'>
                    ...
                    <h3 className='font-semibold text-slate-700 capitalize text-2xl md:text-xl'>
                        {name}</h3>
                ...
                <p className=' text-slate-700 font-medium text-lg'>
                    {category}</p>
                <p className=' font-bold md:text-md md'><span className='text-red-500'>
                    Rs.</span><span>{price}</span></p>
